...
class FaradayEfficiencyCalculator(BaseModel):
    ...
    def _calculate_volumetric_flow_mean(
        self, gc_measurement: Measurement, mean_radius: int = 10
    ) -> float:
        (
            volumetric_flow_datetime_list,
            volumetric_flow_values_list,
        ) = self.experiment.volumetric_flow_time_course
        ...
        match_datetime = min(
            volumetric_flow_datetime_list,
            key=lambda dt: abs(
                datetime.strptime(dt, "%Y-%m-%d %H:%M:%S") - inj_date_datetime
            ),
        )
        ...
        mean_indices_list = [
            i
            for i in range(
                index_match - mean_radius, index_match + mean_radius + 1
            )
        ]
        self._volumetric_flow_mean = np.average(
            [volumetric_flow_values_list[i] for i in mean_indices_list]
        )

    def _calculate_volumetric_fractions(self, assigned_peak_areas_dict: dict):
        self._volumetric_fractions_df = pd.DataFrame(
            index=[species for species in assigned_peak_areas_dict.keys()],
            columns=["Volumetric_fraction"],
        )
        for species, peak_areas in assigned_peak_areas_dict.items():
            self._volumetric_fractions_df.loc[species] = (
                peak_areas[0]
                * self.experiment.get("species_data", "species", species)[0][
                    0
    ...
    def _get_correction_factor(self, species):
        species_data_object = self.experiment.get(
    ...
    def _calculate_real_volumetric_flow(self):
        self._real_volumetric_flow = (
            self._volumetric_flow_mean * self._conversion_factor
        )

    def _calculate_volumetric_flow_fractions(self):
        rename = {"Volumetric_fraction": "Volumetric_flow_fraction"}
        self._volumetric_flow_fractions_df = (
            self._volumetric_fractions_df.multiply(
                self._real_volumetric_flow
    ...
    def _calculate_theoretical_material_flow(self):
        ...
        current_density = (
            absolute_initial_current / self.electrode_surface_area
        )
        factors = {
            "Hydrogen": 2,
            "Carbon monoxide": 2,
    ...
    def calculate_faraday_efficiencies(
        ...
        mean_radius: int,
        assigned_peak_areas_dict: dict,
    ):
        self._calculate_volumetric_flow_mean(
            gc_measurement=gc_measurement, mean_radius=mean_radius
        )
        ...
        faraday_efficiency_df = self._material_flow_df.divide(
