...
# Widget in which only the dataset and scenario can be chosen. After
# confirmation that the selected dataset should be used for the selected
# scenario by pressing a button, a mask should pop up depending on the selected
# scenario.
    # def chose_dataset_and_scenario(self, root: Path, dataset_directory: str) -> None:
    ...
    def choose_data(self, root: Path, dataset_directory: str) -> None:
        ...
        self.scenario_dropdown = widgets.Dropdown(
            options=["", "Selective p-xylol oxidation", "Electrochemical CO2 reduction"],
            description="Choose scenario",
            layout=widgets.Layout(width="auto"),
            style={"description_width": "auto"},
        )
        self.scenario_dropdown.observe(self._scenario_input_handler, names="value")
        widgets1 = widgets.HBox([self.scenario_dropdown])

        layout = widgets.HBox([widgets1])
        display(layout)

    def _scenario_input_handler(self, _):
        if self.scenario_dropdown.value == "Selective p-xylol oxidation":
            self._scenario1()
        elif self.scenario_dropdown.value == "Electrochemical CO2 reduction":
            self._scenario2()

    def _scenario1(self):
        self.full_layout.close()
        print("Scenario 1")
        self.pid_output = widgets.Output()
        self.datasets = self.librarian.search_files_in_subdirectory(
            root_directory=self.root,
            directory_keys=[self.dataset_directory],
            file_filter="json",
            verbose=False,
        )
        self.dataset_dropdown = widgets.Dropdown(
            options=[("", Path(""))]
            + [(path.parts[-1], path) for _, path in self.datasets.items()],
            description="Choose dataset",
            layout=widgets.Layout(width="auto"),
            style={"description_width": "auto"},
        )
