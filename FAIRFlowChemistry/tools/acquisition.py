...
# Objects
from FAIRFlowChemistry.core import Dataset, Experiment, PlantSetup
...
class measurement_object:
    ...
    def __init__(self, name: str, component_list: List[str]) -> None:
        ...
        self.galvano_component = widgets.Dropdown( options=[""]+component_list,
                                                   description="Corresponding plant component",
                                                   layout=widgets.Layout(flex=str(portion)),
                                                   style={'description_width': 'auto'})
        
        self.gc_component      = widgets.Dropdown( options=[""]+component_list,
                                                   description="Corresponding plant component",
                                                   layout=widgets.Layout(flex=str(portion)),
                                                   style={'description_width': 'auto'})
        
        self.mfm_component     = widgets.Dropdown( options=[""]+component_list,
                                                   description="Corresponding plant component",
                                                   layout=widgets.Layout(flex=str(portion)),
                                                   style={'description_width': 'auto'})

        self.galvano_files = widgets.TagsInput(allow_duplicates=False, layout=widgets.Layout(flex=str(10-portion)))
        self.GC_files      = widgets.TagsInput(allow_duplicates=False, layout=widgets.Layout(flex=str(10-portion)))
        self.MFM_files     = widgets.TagsInput(allow_duplicates=False, layout=widgets.Layout(flex=str(10-portion)))

        widgets0  = widgets.VBox([widgets.Label(value='Files for galvanostat:'), widgets.HBox([self.galvano_files,self.galvano_component])])
        widgets1  = widgets.VBox([widgets.Label(value='Files for gas chromatography:'), widgets.HBox([self.GC_files,self.gc_component])])
        widgets2  = widgets.VBox([widgets.Label(value='Files for mass flow meter:'), widgets.HBox([self.MFM_files,self.mfm_component])])

        # Combine the layout
        self.full_layout = widgets.VBox([widgets0,widgets1,widgets2])

    def update_component_list(self, component_list: List[str]):
        self.galvano_component.options = [""]+component_list
...
class reading_raw_data_widget():
    ...
    def add_experiment(self,_):
        ...
        for measurement in self.measurement_objects:
            ...
            pot_measurements = [ gstatic_parser( metadata_path = potentiostat_file ) for potentiostat_file in measurement.galvano_files.value ] 
            mfm_measurements = [ mfm_parser( experimental_data_path = mfm_file ) for mfm_file in measurement.MFM_files.value ] 
            gc_measurements  = [ gc_parser( metadata_path = measurement.GC_files.value[i], experimental_data_path = measurement.GC_files.value[i+1] )
            ...
            for i,pm in enumerate(pot_measurements):
                ...
                if measurement.galvano_component.value in self.component_list:
                    pm.source = self.plant.components[ self.component_list.index( measurement.galvano_component.value ) ]
            ...
            for i,mm in enumerate(mfm_measurements):
                ...
                if measurement.mfm_component.value in self.component_list:
                    mm.source = self.plant.components[ self.component_list.index( measurement.mfm_component.value ) ]

            for i,gm in enumerate(gc_measurements):
                gm.id     = f'{measurement.name.replace(" ", "")}_gaschromatography{i}'

                # Add defined source of measurement
                if measurement.gc_component.value in self.component_list:
                    gm.source = self.plant.components[ self.component_list.index( measurement.gc_component.value ) ]
    ...
    def experiment_input_handler(self,_):
        ...
        del_idx = [ i for i,exp in enumerate(self.dataset.experiments) if not exp.id in self.experiments.value ]
        del_idx.sort( reverse = True)
    ...
    def choose_data(self, root: Path, dataset_directory: str) -> None:
        ...
        datasets                = self.librarian.search_files_in_subdirectory( root_directory=root, directory_keys=[dataset_directory],
                                                                               file_filter="json", verbose=False)
        ...
        self.button_read_pid    = widgets.Button( description='Read PID',
                                                  layout=widgets.Layout(width='auto'))
 
        self.button_vis_pid     = widgets.Button( description='Visualize PID',
                                                  layout=widgets.Layout(width='auto'))
        ...
        widgets2  = widgets.VBox([self.pid_file, widgets.HBox([self.button_read_pid, self.button_vis_pid ]), self.pid_output, v_space_s, self.species_file,v_space_s])
        ...
        widgets5 = widgets.VBox([widgets.VBox([widgets.Label(value='After selecting all necessary files for an experiment, add the experiment to the chosen dataset.'),
                                                self.experiment_name, self.button_add_exp]), 
                                 widgets.VBox([widgets.Label(value='Experiments:'),self.experiments])])
        widgets6 = widgets.VBox([self.button_save ],layout=widgets.Layout(align_items = 'center'))

        # Combine the layout
        full_layout = widgets.VBox([widgets0,v_space,widgets1,widgets2,widgets3,widgets4,v_space,widgets5,v_space,widgets6])
